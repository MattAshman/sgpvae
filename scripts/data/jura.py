...
def load():
    ...
    # Load training data.
    columns = pd.read_csv(data_path('jura', 'prediction.dat'),
                          skiprows=2,
                          header=None,
                          nrows=11)
    columns = [name_corr[c] if c in name_corr else c for c in columns[0]]
    train = pd.read_csv(data_path('jura', 'prediction.dat'),
                        skiprows=13,
                        header=None,
                        sep=r'\s+')
...
def _fetch():
    import pdb
    pdb.set_trace()
    resource(target=data_path('jura', 'prediction.dat'),
             url='https://drive.google.com/uc'
                 '?export=download&id=0B6subVejOkD_Vm8teTQtWjB3QzQ')
    resource(target=data_path('jura', 'validation.dat'),
             url='https://drive.google.com/uc'
