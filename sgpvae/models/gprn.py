...
class GPRN(nn.Module):
    """The GPRN model.

    :param encoder: encoder network.
    :param f_dim (int): the dimension of latent space.
    :param y_dim (int): the dimension of the output.
    :param f_kernel: GP kernel over function values.
    :param w_kernel: GP kernel over weights.
    :param output_sigma (float): initial output sigma.
    :param add_jitter (bool, optional): whether to add jitter to the GP prior
    covariance matrix.
    """
    def __init__(self, encoder, f_dim, y_dim, f_kernel, w_kernel,
                 output_sigma=1., add_jitter=False):
        super().__init__()

        self.encoder = encoder
        self.f_dim = f_dim
        self.y_dim = y_dim
        self.output_sigma = nn.Parameter(output_sigma)
        ...
        if not isinstance(w_kernel, list):
            w_kernels = [copy.deepcopy(w_kernel) for _ in range(f_dim * y_dim)]
            self.w_kernels = KernelList(w_kernels)

        else:
            assert len(w_kernel) == y_dim, 'Number of kernels must be equal ' \
    ...
    def w_prior(self, x, diag=False):
        """Return latent prior mean and covariance."""
        mw = torch.zeros((self.f_dim * self.y_dim), x.shape[0])
    ...
    def predict_y(self, x, y, **kwargs):
        ...
        # TODO: can analytically compute the predictive mean.
        ...
        for f, w in zip(f_samples, w_samples):
            ...
            y_mu = torch.zeros(self.y_dim, x.shape[0])
            for n in range(x.shape[0]):
                wn = w[:, n].reshape(self.y_dim, self.f_dim)
                # wn = torch.sigmoid(wn)
                y_mu[:, n] = wn.matmul(f[:, n])

            y_mus.append(y_mu)
            y_samples.append(y_mu + self.output_sigma * torch.randn_like(y_mu))
...
class SGPRN(GPRN):
    ...
    :param f_dim (int): the dimension of latent space.
    :param y_dim (int): the dimension of the first hidden layer,
    ...
    points.
    """
    def __init__(self, encoder, decoder, f_dim, y_dim, f_kernel, w_kernel, z,
                 add_jitter=False, fixed_inducing=False):
        super().__init__(encoder, decoder, f_dim, y_dim, f_kernel, w_kernel,
                         add_jitter)
