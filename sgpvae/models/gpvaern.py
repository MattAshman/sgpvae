...
__all__ = ['GPVAERN', 'SGPVAERN']

JITTER = 1e-5


class GPVAERN(nn.Module):
    """The GP-VAE-RN model.

    :param encoder: encoder network.
    :param decoder: decoder network.
    :param f_dim (int): the dimension of latent space.
    :param w_dim (int): the dimension of the final hidden layer.
    :param y_dim (int): the dimension of the output, whose weights are 
    distributed according to a GP.
    ...
    covariance matrix.
    """
    def __init__(self, encoder, decoder, f_dim, w_dim, y_dim, f_kernel,
                 w_kernel, add_jitter=False):
        super().__init__()
        ...
        if not isinstance(w_kernel, list):
            w_kernels = [copy.deepcopy(w_kernel) for _ in range(w_dim * y_dim)]
            self.w_kernels = KernelList(w_kernels)

        else:
            assert len(w_kernel) == w_dim * y_dim, 'Number of kernels must ' \
    ...
    def w_prior(self, x, diag=False):
        """Return latent prior mean and covariance."""
        mw = torch.zeros((self.w_dim * self.y_dim), x.shape[0])
    ...
    def predict_y(self, x, y, **kwargs):
        ...
        for f, w in zip(f_samples, w_samples):
            ...
            g = torch.zeros(self.y_dim, x.shape[0])
