...
__all__ = ['MoGGPVAE', 'MoGSGPVAE']

JITTER = 1e-5


class MoGGPVAE(VAE):
    """The GP-VAE model with mixture of Gaussians approximate posterior.
    ...
    :param c (int): the number of mixture components.
    :param add_jitter (bool, optional): whether to add jitter to the GP prior
    covariance matrix.
    """
    def __init__(self, likelihood, variational_dist, latent_dim, kernel, c,
                 add_jitter=False):
        ...
        # Mixing coefficients in the variational approximation.
        self._log_mixing_coeffs = nn.Parameter((torch.ones(c)/c).log(),
                                               requires_grad=True)
    ...
    def mixing_coeffs(self):
        return self._log_mixing_coeffs.exp() / sum(
            self._log_mixing_coeffs.exp())
    ...
    def qf(self, x=None, y=None, pf=None, lf=None, mask=None, diag=False,
        ...
        # Reshape to [C, K, N].
        lf_mu = lf.mean.transpose(0, 1).reshape(self.c, self.latent_dim, -1)
        lf_sigma = lf.stddev.transpose(0, 1).reshape(
            self.c, self.latent_dim, -1)
        ...
        # B = La^{-1} * Lf^{-1} * \mu_{\phi}.
        b = torch.triangular_solve(lf_mu.unsqueeze(-1), pf_chol,
                                   upper=False)[0]
        ...
        if x_test is not None:
            ...
            qf = MultivariateNormal(qf_mu.squeeze(-1), qf_cov)

            return qf

    def kl_divergence(self, qf, pf):
        """Computes a lower bound between the MoG distribution q(f) and the
        Gaussian p(f)."""
        # Lower bound the entropy using Jensen's inequality.
        # [C, C, K, N, N]
        covs = (qf.covariance_matrix.unsqueeze(1) +
                qf.covariance_matrix.unsqueeze(0))
        # [C, K, N]
        means = qf.mean

        dists = MultivariateNormal(means, covs)

        # [K]
        entropy = (self.mixing_coeffs * torch.logsumexp(
            (self.mixing_coeffs*dists.log_prob(
                qf.mean).transpose(0, -1)).transpose(0, -1),
            dim=0).transpose(0, -1)).transpose(0, -1).sum(0)

        # Compute the cross-entropy term.
        pf_chol = pf.covariance_matrix.cholesky()
        pf_precision = torch.stack([
            torch.cholesky_inverse(chol, upper=False) for chol in pf_chol])

        # [K]
        pf_cov_logdets = 2 * torch.stack([chol.diag() for chol in
                                          pf_chol]).log().sum(-1)

        # [C, K]
        qf_mu_term = qf.mean.unsqueeze(-2).matmul(pf_precision.matmul(
            qf.mean.unsqueeze(-1))).squeeze(-1).squeeze(-1)

        # [C, K]
        trace_term = torch.einsum('abii->ab', pf_precision.matmul(
            qf.covariance_matrix))

        # [K]
        cross_entropy = 0.5 * (self.mixing_coeffs * (
                qf.event_shape[0] * np.log(2*np.pi) + pf_cov_logdets +
                qf_mu_term + trace_term).transpose(0, 1)).sum(1)

        kl = entropy + cross_entropy
        return kl

    def elbo(self, x, y, mask=None, mask_q=None, num_samples=1, n_train=None):
        ...
        # KL(q(f) || p(f)) term.
        kl = self.kl_divergence(qf, pf).sum()

        # log p(y|f) term.
        qf_marginals = Normal(qf.mean, qf.variance**0.5)
        f_samples = qf_marginals.rsample((num_samples,))

        log_py_f = 0
        for f in f_samples:
            if mask is not None:
                log_py_f += (self.mixing_coeffs * (self.likelihood.log_prob(
                    f.transpose(-1, -2), y) * mask).sum(-1).sum(-1)).sum()
            else:
                log_py_f += (self.mixing_coeffs * self.likelihood.log_prob(
                    f.transpose(-1, -2), y).sum(-1).sum(-1)).sum()
    ...
    def predict_y(self, x, y=None, mask=None, x_test=None, num_samples=1):
        ...
        if y is None:
            ...
            dists = [MultivariateNormal(mu, cov) for (mu, cov) in zip(
                qf.mean, qf.covariance_matrix)]
            mix = Categorical(self.mixing_coeffs).sample((num_samples,))
            f_samples = torch.stack([dists[k].sample() for k in mix], dim=0)
...
class MoGSGPVAE(MoGGPVAE):
    """The SGP-VAE model with mixture of Gaussians approximate posterior.
    ...
    points.
    """
    def __init__(self, likelihood, variational_dist, latent_dim, kernel, c, z,
                 add_jitter=False, fixed_inducing=False):
        super().__init__(likelihood, variational_dist, latent_dim, kernel, c,
                         add_jitter)
    ...
    def qf(self, x, y=None, pu=None, lf=None, mask=None, diag=False,
        ...
        if x_test is not None:
            ...
            qu_mu = torch.triangular_solve(c, b_chol.transpose(-1, -2),
                                           upper=True)[0]
            qu_mu = pu_chol.matmul(qu_mu).squeeze(-1)
    ...
    def elbo(self, x, y, mask=None, num_samples=1, n_train=None):
        ...
        # KL(q(u) || p(u)) term.
        kl = self.kl_divergence(qu, pu).sum()
